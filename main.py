...
app.add_middleware(
    ...
    allow_origins=[
        "http://localhost:3000", 
        "http://127.0.0.1:3000",
        "https://your-spectraine-frontend.vercel.app",
        "https://spectraine.com"
    ],
...
def generate_instances():
    ...
    instances = []
    # Generate 12-18 instances for realistic enterprise environment
...
async def free_assessment(contact: AssessmentRequest):
    """Submit request for free threat assessment"""
    print(f"🎯 NEW ASSESSMENT REQUEST:")
    print(f"   Name: {contact.name}")
    print(f"   Company: {contact.company}")
    print(f"   Email: {contact.email}")
    print(f"   AWS Spend: {contact.aws_spend}")
    print(f"   Priority Concerns: {contact.priority_concerns}")
